...
char *nom,*mdp,*code,*mdpsav,*nomalbum,*estpublic,*horodatage,*jour,*mois,*annee,*heure,*minute,*seconde;
time_t tim;
struct tm *TM;
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
         	...
         	if(success==1)
         		...
         		sprintf(query,"select * from album where compte=\'%s\' and nom=\'%s\';",nom,nomalbum);
         		db_query(handler,query);
         		if(db_ntuples(result)!=0)
         			{
         			strcpy(reponse,"Echec de l'enregistrement. Cet album existe déjà");
         			db_clear_result(result);
         			}
         		else
         			{
         			db_clear_result(result);
						sprintf(query,"insert into album values (\'%s\',\'%s\',\'%s\',\'%s\');",nom,nomalbum,estpublic,horodatage);
						db_query(handler,query);
						db_clear_result(result);
						sprintf(query,"select * from album where compte=\'%s\' and nom=\'%s\';",nom,nomalbum);
						db_query(handler,query);
						if(db_ntuples(result)==1) strcpy(reponse,"L'album a bien été enregistré");
						else strcpy(reponse,"Echec de l'enregistrement");
						db_clear_result(result);
						}
					}				
				db_close(handler);
