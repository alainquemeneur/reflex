	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
				...
				if(success==1)
					{
					sprintf(query,"select nom,public from album where compte=\'%s\' order by horodatage desc;",nom);
					db_query(handler,query);
					}
				}
			}
...
if(success==1)
	...
	printf("<div data-role=\"navbar\">\n\
      ...
      <form autocomplete=\"off\" action=\"/cgi-bin/reflex_doeditalbum.cgi\" method=\"post\" id=\"form\" data-transition=\"none\" data-rel=\"dialog\">\n\
      		<input type=\"hidden\" name=\"txt-compte\" id=\"txt-compte\" value=\"%s\">\n\
            <input type=\"hidden\" name=\"txt-password\" id=\"txt-password\" value=\"%s\">\n\
            <input type=\"hidden\" name=\"txt-code\" id=\"txt-code\" value=\"%s\">\n\
            <fieldset data-role=\"controlgroup\">\n\
            <legend>Nom de l'album à modifier</legend>\n",nom,mdp,code,nom,nom,mdp,code);
        		for(i=0;i<db_ntuples(result);i++)
        			...
        			if(i==0) printf("<input type=\"radio\" name=\"radio-choice-e\" id=\"radio-choice-e%d\" value=\"%s\" data-theme=\"b\" checked=\"checked\">\n\
            <label for=\"radio-choice-e%d\">%s (%s)</label>\n",i,reponse,i,reponse,reponse2);
					else printf("<input type=\"radio\" name=\"radio-choice-e\" id=\"radio-choice-e%d\" value=\"%s\" data-theme=\"b\">\n\
            <label for=\"radio-choice-e%d\">%s (%s)</label>\n",i,reponse,i,reponse,reponse2);
					}
				db_clear_result(result);
				printf("</fieldset>\n\
            <button type=\"submit\" data-mini=\"false\" data-inline=\"false\" data-theme=\"c\" data-icon=\"edit\">Modifier l'album</button>\n\
         </form>\n\
      </div>\n\
