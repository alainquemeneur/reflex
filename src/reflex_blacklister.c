	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
         	...
         	if(success==1)
         		{
         		sprintf(query,"select * from blacklist where blacklister=\'%s\' and blacklisted=\'%s\';",nom,owner);
         		db_query(handler,query);
         		if(db_ntuples(result)==0)
         			{
         			db_clear_result(result);
         			sprintf(query,"insert into blacklist values (\'%s\',\'%s\');",nom,owner);
         			db_query(handler,query);
         			db_clear_result(result);
         			sprintf(reponse,"Vous bloquez désormais %s",owner);
         			}
         		else
         			{
         			db_clear_result(result);
         			sprintf(query,"delete from blacklist where blacklister=\'%s\' and blacklisted=\'%s\';",nom,owner);
         			db_query(handler,query);
         			db_clear_result(result);
         			sprintf(reponse,"Ok, vous ne bloquez plus %s",owner);
         			}
					}				
				db_close(handler);
				}
